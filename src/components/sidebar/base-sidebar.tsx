...
export function SidebarMenuLinkItem({
  ...
  const match = useMatch({ from: matchFrom as never, shouldThrow: false });

  if (!condition) return null;

  return (
    <SidebarMenuItem>
      <SidebarMenuButton isActive={!!match} render={<Link to={to} />}>
        <Icon />
        <span>{label}</span>
      </SidebarMenuButton>
    </SidebarMenuItem>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="mt-4">
          <Table>
            ...
            <TableBody>
              {sessions?.data?.map((s: SessionItem) => (
                <TableRow
                  key={s.session.id}
...
export function BaseSidebar({ header, children }: BaseSidebarProps) {
  return (
